...
Relic_AnnounceEnabled = (Relic_AnnounceEnabled ~= false)
Relic_AnnounceGroupOnly = Relic_AnnounceGroupOnly or false
Relic_AnnounceDelay = Relic_AnnounceDelay or 1
...
local Relic_View, Relic_Textures = Relic_View, {"Interface\\AddOns\\Ogrila_Stone\\UI\\Shard_g", "Interface\\AddOns\\Ogrila_Stone\\UI\\Shard_y", "Interface\\AddOns\\Ogrila_Stone\\UI\\Shard_b", "Interface\\AddOns\\Ogrila_Stone\\UI\\Shard_r"}

local Relic_Chain, Relic_DebuffExpire, Relic_DefaultKeyset = {}, 0, L.colorDefaultKeys
local Relic_ColorShortcuts = {"V", "J", "B", "R"}
local Relic_LastAnnounceTime = 0

local function Relic_AnnounceSequence()
	if not Relic_AnnounceEnabled then return end

	local currentTime = GetTime()
	if currentTime - Relic_LastAnnounceTime < (Relic_AnnounceDelay or 1) then
		return
	end
	Relic_LastAnnounceTime = currentTime

	if Relic_AnnounceGroupOnly and not IsInGroup() then return end

	if #Relic_Chain == 0 then
		print("[OGRILA] Séquence vidée")
		return
	end

	local sequence = ""
	for i=1, #Relic_Chain do
		sequence = sequence .. (i > 1 and "/" or "") .. Relic_ColorShortcuts[Relic_Chain[i]]
	end

	print("[OGRILA] Reliques : " .. sequence)
end
...
for i=2,10 do
	Relic_View.replay[i]:SetPoint("TOPLEFT", Relic_View.replay[i-1], "TOPRIGHT", -4, 0)
end
Relic_View.replay[6]:ClearAllPoints()
Relic_View.replay[6]:SetPoint("TOP", Relic_View.replay[1], "BOTTOM", -4, -2
)
...
SLASH_RELIC1, SLASH_RELIC2, SLASH_RELIC3, SlashCmdList.RELIC = "/ogri", "/relic", "/ol", function(msg)
	if msg == "config" then
		RelicConfigFrame:SetShown(not RelicConfigFrame:IsShown())
	elseif msg == "announce on" then
		Relic_AnnounceEnabled = true
		print("[OGRILA] Annonces activées")
	elseif msg == "announce off" then
		Relic_AnnounceEnabled = false
		print("[OGRILA] Annonces désactivées")
	elseif msg == "group on" then
		Relic_AnnounceGroupOnly = true
		print("[OGRILA] Annonces seulement en groupe")
	elseif msg == "group off" then
		Relic_AnnounceGroupOnly = false
		print("[OGRILA] Annonces en solo autorisées")
